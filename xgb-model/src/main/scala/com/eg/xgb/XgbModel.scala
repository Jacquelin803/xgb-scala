...
class XgbModel {
  def train(spark:SparkSession,rawSamples: DataFrame): Unit = {
    ...
    Files.write(Paths.get("/opt/temp/xgb_cv.pmml"), pmmlBytes)
...
//    /opt/workspace/xgb-scala/xgb-model/src/main/resources/iris.csv
    val rawSamples = spark.read.format("csv").option("sep", ",").option("inferSchema", "true").option("header", "true").load("xgb-model/src/main/resources/iris.csv")
